...
Model::Model(ID3D11Device* dev, const char* const _modelName)
{
	...
	ModelTools::ComputeTangent(pStdVerts, numVerts, pTriList, numTris, true);
...
Model::Model(ID3D11Device* dev, Model::PreMadeModels pm)
{
	...
	case UnitPyramidRepeatedTexture:
		...
		break;
	case UnitSphere:
		ModelTools::CreateUnitSphere(pStdVerts, numVerts, pTriList, numTris);
		break;
	default:
