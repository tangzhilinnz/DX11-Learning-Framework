...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    float3 normalMapSample = normalMap.Sample(aSampler, input.Tex).xyz;
    // Unpack the normal from [0, 1] to [-1, 1]
    float3 normalTS = normalMapSample * 2.0f - float3(1.f, 1.f, 1.f);
    ...
    float3 norm = normalize(mul(normalTS, TBN));
    ...
    // ***************** Light section ***********************/
    float center = (8.85f + 9.63f) * 0.5f;
    float range = 9.63f - 8.85f;
    float halfRange = range * 0.5f;

    // Normalized distance from center (0 at center, 1 at edges)
    float dist = abs(input.PosMS.z - center) / halfRange;

     // Power curve falloff (higher exponent = steeper falloff)
    float falloff = 1.0 - pow(saturate(dist), 8.0f);

    // Apply to light
    litColor += float4(3.5f, 3.5f, 3.5f, 1.0f) * falloff;
    // ***************** Light section ***********************/
